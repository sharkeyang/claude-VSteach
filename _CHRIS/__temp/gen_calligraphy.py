...
GUWEN_DB = {
    ...
    "三峡": "自三峡七百里中，两岸连山，略无阙处。重岩叠嶂，隐天蔽日，自非亭午夜分，不见曦月。至于夏水襄陵，沿溯阻绝。或王命急宣，有时朝发白帝，暮到江陵，其间千二百里，虽乘奔御风，不以疾也。春冬之时，则素湍绿潭，回清倒影，绝巘多生怪柏，悬泉瀑布，飞漱其间，清荣峻茂，良多趣味。每至晴初霜旦，林寒涧肃，常有高猿长啸，属引凄异，空谷传响，哀转久绝。故渔者歌曰：巴东三峡巫峡长，猿鸣三声泪沾裳。",
    ...
    "天将降大任于是人也": "故天将降大任于是人也，必先苦其心志，劳其筋骨，饿其体肤，空乏其身，行拂乱其所为，所以动心忍性，曾益其所不能。人恒过，然后能改。困于心，衡于虑，而后作。征于色，发于声，而后喻。入则无法家拂士，出则无敌国外患者，国恒亡。然后知生于忧患而死于安乐也。",
    "北冥有鱼": "北冥有鱼，其名为鲲。鲲之大，不知其几千里也。化而为鸟，其名为鹏。鹏之背，不知其几千里也；怒而飞，其翼若垂天之云。是鸟也，海运则将徙于南冥。南冥者，天池也。《齐谐》者，志怪者也。《谐》之言曰：鹏之徙于南冥也，水击三千里，抟扶摇而上者九万里，去以六月息者也。",
    "虽有嘉肴": "虽有嘉肴，弗食，不知其旨也；虽有至道，弗学，不知其善也。是故学然后知不足，教然后知困。知不足，然后能自反也；知困，然后能自强也。故曰：教学相长也。《兑命》曰学学半，其此之谓乎！",
}
...
# 计划编号（来自暑假方案「小升初必背」列表，课内1-16篇已完成）
SEQUENCE = {
    "论语六章": 17,
    "陋室铭": 18,
    "爱莲说": 19,
...
def save_to_pdf(title, text):
    ...
    # 计算序号（优先用计划编号，不在列表中的按拼音排序）
    seq = SEQUENCE.get(title, 0)
    if seq == 0:
        names = sorted(GUWEN_DB.keys())
        seq = names.index(title) + 1
    prefix = f'{seq:02d}'

    # 保存 HTML
    safe = title.replace('《','').replace('》','').replace(' ','')
    html_path = os.path.join(OUT_DIR, f'__temp/字帖_{prefix}_{safe}.html')
    pdf_path  = os.path.join(OUT_DIR, f'字帖_{prefix}_{safe}.pdf')
...
def list_guwen():
    print('可用的古文（计划编号 / 拼音排序）：')
    for i, name in enumerate(sorted(GUWEN_DB), 1):
        text = GUWEN_DB[name]
        chars = sum(1 for c in text if '一' <= c <= '鿿')
        seq = SEQUENCE.get(name, i)
        print(f'  {seq:3d}. {name} ({chars}字)  {f"#{i}" if seq != i else ""}')
    print(f'\n共 {len(GUWEN_DB)} 篇')
    print(f'\n共 {len(GUWEN_DB)} 篇')
